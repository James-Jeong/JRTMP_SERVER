...
public class RtmpRegisterReq extends URtmpMessage {

    private static final Logger log = LoggerFactory.getLogger(RtmpRegisterReq.class);
    ...
    public RtmpRegisterReq(byte[] data) throws URtmpException {
        if (data.length >= URtmpHeader.U_RTSP_HEADER_SIZE + ByteUtil.NUM_BYTES_IN_LONG + ByteUtil.NUM_BYTES_IN_SHORT) {
            int index = 0;
    ...
    public RtmpRegisterReq(String magicCookie, URtmpMessageType messageType, int seqNumber, long timeStamp, String id, long expires, short listenPort) {
