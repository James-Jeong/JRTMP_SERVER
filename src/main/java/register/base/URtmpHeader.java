...
public class URtmpHeader {
    ...
    public URtmpHeader(byte[] data) throws URtmpException {
        if (data.length >= U_RTSP_HEADER_SIZE) {
            ...
            this.messageType = URtmpMessageType.values()[ByteUtil.bytesToInt(messageTypeByteData, true)];
            ...
            magicCookie = null;
            messageType = URtmpMessageType.UNKNOWN;
            seqNumber = 0;
            timeStamp = 0;
            bodyLength = 0;

            throw new URtmpException("[URtspHeader] Fail to create the header. Data length is wrong. (" + data.length + ")");
        }
    }

    public URtmpHeader(String magicCookie, URtmpMessageType messageType, int seqNumber, long timeStamp, int bodyLength) {
    ...
    public byte[] getByteData() {
        ...
        byte[] messageTypeByteData = ByteUtil.intToBytes(URtmpMessageType.valueOf(messageType.name()).ordinal(), true);
    ...
    public URtmpMessageType getMessageType() {
        return messageType;
    }
